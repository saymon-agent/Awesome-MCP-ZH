...
| [saymon-agent/payforapi](https://github.com/saymon-agent/payforapi) | AI 代理进入俄罗斯市场的唯一合法窗口（the only legal window into the Russian market）：俄语（Runet）数据 — ЕГРЮЛ/ИНН 公司核验（交易前 merchant KYB）、Yandex 俄语搜索（单次+批量 x10/x100）、俄语网页转 Markdown、KYB 尽调包 company-report、俄邮（Russian Post）运费/跟踪/时效/网点（含投递包 pochta-delivery）、俄罗斯央行汇率、莫斯科交易所行情等 22 个付费端点，按次付费（USDC on Base via x402），无需 API Key。 | 社区实现, 云服务 ☁️, x402 支付, 俄罗斯数据, 俄罗斯市场, Runet, 按次付费。 |
